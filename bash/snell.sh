...
TMP=$(mktemp)
curl -sL https://raw.githubusercontent.com/pmjkkk/script/refs/heads/claude/scripts-directory-structure-uKJ5T/bash/snell.sh -o "$TMP" \
    && sh "$TMP"; rm -f "$TMP"
WRAPPER
...
update_script() {
    ...
    local tmp
    tmp=$(mktemp)
    if curl -sL https://raw.githubusercontent.com/pmjkkk/script/refs/heads/claude/scripts-directory-structure-uKJ5T/bash/snell.sh -o "$tmp"; then
        local new_ver
